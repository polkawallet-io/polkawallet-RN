...
class Transfer_details extends PureComponent {
   ...
   */
  open = () => {
    Linking.openURL('https://scan.chainx.org/txs/0x' + String(this.state.data.hash))
  }
    ...
    return (
      <SafeAreaView style={[styles.container]}>
        ...
        >
          ...
          >
            {this.state.data.hash + ':' + i18n.t('Assets.Successed')}
          </Text>
          {/* 日期 */}
          <Text
            style={{
          ...
          >
            {moment(this.state.data.time).format('DD/MM/YYYY HH:mm:ss')}
          </Text>
        </View>
        <View
          style={{
        ...
        >
          ...
          <View style={styles.list_View}>
            ...
            <Text style={[styles.list_text2, { fontSize: 16 }]}>{String(this.state.data.value / 100000000)}</Text>
          </View>
          {/* Fees */}
          <View style={styles.list_View}>
            <Text style={styles.list_text1}>{i18n.t('Assets.memoOf')}:</Text>
            <Text style={styles.list_text2}>{String(this.state.data.memo)} </Text>
          </View>
          {/* To */}
          <View style={styles.list_View}>
            ...
            <View style={styles.grey_text}>
              <Text style={styles.address} ellipsizeMode="middle" numberOfLines={1}>
                {chainxAPI.getAddress(this.state.data.payee)}
              </Text>
            </View>
            <TouchableOpacity
              onPress={() => {
          ...
          <View style={styles.list_View}>
            ...
            <View style={styles.grey_text}>
              <Text style={[styles.address, { fontSize: 14 }]} ellipsizeMode="middle" numberOfLines={1}>
                {chainxAPI.getAddress(this.state.data.signed)}
              </Text>
            </View>
          ...
          <View style={styles.list_View}>
            ...
            <Text style={[styles.list_text2, { fontSize: 16 }]}>#{this.state.data.number}</Text>
          </View>
          {/* BlockHash */}
          <View style={styles.list_View}>
          ...
          <TouchableWithoutFeedback onPress={this.open}>
            <View style={styles.viewForText}>
              <Text style={{ color: '#3385ff' }}>View in the Chainx.org</Text>
            </View>
          </TouchableWithoutFeedback>
          <View style={{ justifyContent: 'center', alignItems: 'center' }}>
            ...
            >
              <QRCode
                value={this.state.url + String(this.state.data.number)}
                size={75}
                bgColor="black"
                fgColor="white"
              />
