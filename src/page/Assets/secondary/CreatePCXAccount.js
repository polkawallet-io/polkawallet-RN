...
class CreateAccount extends Component {
  ...
  componentDidMount() {
    InteractionManager.runAfterInteractions(() => {
      ;(async () => {
        let menemonic = await Account.newMnemonic()
        let useraccount = Account.from(menemonic)
        this.pair = {
          type: 'ed25519',
          address: useraccount.address(),
          meta: {},
          isLocked: false,
          publickKey: useraccount.publicKey(),
          privateKey: useraccount.privateKey(),
          type: 2
        }
        //let balance = await chainxAPI.freeBalance(useraccount.address())
        this.setState({
          key: menemonic,
          balance: 0,
          address: useraccount.address(),
  ...
  async Modify_way(way_change) {
    ...
    if (way_change == 'Keystore') {
      ...
      key = await Account.newMnemonic()
      address = Account.from(key).address()
    } else {
      console.warn('生成私钥...')
      let menemonic = await Account.newMnemonic()
      key = Account.from(menemonic).privateKey()
      address = Account.from(menemonic).address()
    }
    this.setState({
      key: key,
      address: address
  ...
  onChangekey(Changekey) {
    ...
    if (this.state.way == 'Mnemonic' || this.state.way == 'Mnemonic24') {
      try {
        ...
        this.setState({
          key: Changekey,
          address: Account.from(Changekey).address()
        })
      } catch (e) {
        tag = false
        this.setState({
      ...
      if (this.state.password != this.state.passwordErepeat) {
        ...
        SInfo.getItem(this.state.address, {
          ...
          if (this.state.way == 'Keystore') {
            ...
            this.pair.meta.name = this.state.name
            this.json = Account.from(this.state.key).encrypt(this.state.password)
            this.json.meta = {}
            this.json.meta.name = this.pair.meta.name
            this.json.meta.type = 2
            this.json.address = this.state.address
            ...
            this.props.rootStore.stateStore.isfirst = 1
            this.props.rootStore.stateStore.type = 2
            this.props.rootStore.stateStore.Accounts.push({
